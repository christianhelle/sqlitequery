<#
.SYNOPSIS
    Build script for SQLite Query Analyzer

.DESCRIPTION
    Builds SQLite Query Analyzer using CMake and Ninja on Windows, Linux, or macOS.

.PARAMETER Package
    Create installer/package after build

.PARAMETER QtPath
    Path to Qt installation (default: C:\Qt\6.9.0\msvc2022_64 or $env:QT_PATH)
...
.EXAMPLE
    .\build.ps1 -Package
    Build in Release mode and create installer package

.EXAMPLE
    .\build.ps1 -Configuration Debug -QtPath "C:\Qt\6.8.0\msvc2022_64"
    Build in Debug mode with custom Qt path
#>
...
if ($IsWindows)
{
  ...
  & cmd /c "`"$vcvars`" && cmake . -G `"$generator`" -DCMAKE_PREFIX_PATH=$QtPath -DCMAKE_CXX_STANDARD=17 -DCMAKE_CXX_FLAGS=`"/Zc:__cplusplus /permissive-`" -DCMAKE_BUILD_TYPE=$Configuration -B build"
  ...
  & cmd /c "`"$vcvars`" && cmake --build build --config $Configuration --parallel 32"
  if ($LASTEXITCODE -ne 0)
  {
  ...
  New-Item -ItemType Directory -Path ".\build\$Configuration" -Force
  Copy-Item .\build\SQLiteQueryAnalyzer.exe ".\build\$Configuration\SQLiteQueryAnalyzer.exe"
  & "$QtPath\bin\windeployqt.exe" ".\build\$Configuration\SQLiteQueryAnalyzer.exe"
...
if ($IsMacOS -And $Package)
{
  ...
  cmake -B build -DCMAKE_BUILD_TYPE=$Configuration -DCMAKE_INSTALL_PREFIX=/tmp/sqlitequery
  cmake --build build --config $Configuration --parallel 32
  if ($Package)
  {
