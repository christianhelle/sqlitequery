...
public:
    ...
    virtual void execute(IDatabase *db, const CancellationToken *token, ExportDataProgress *progress) = 0;
};

class SqlExportStrategy : public ExportStrategy {
public:
    SqlExportStrategy(DatabaseInfo info, QString filepath);
    void execute(IDatabase *db, const CancellationToken *token, ExportDataProgress *progress) override;
private:
    DatabaseInfo info_;
    QString filepath_;
};
