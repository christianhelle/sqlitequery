...
- **Zoom** — a scale factor the user applies to the text of a pane, in discrete
  ...
  size. There are two: one the query editor and the result Messages pane share,
  and one the Tree carries, so either side can be enlarged without the other.
  Part of the window state, so both are persisted between runs.
